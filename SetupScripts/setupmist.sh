...
echo "$CASE_NUM"
mkdir "$PARENT_DIR/mist_data/case_$CASE_NUM"

PATIENT_DIR="$PARENT_DIR/mist_data/case_$CASE_NUM"

if c3d "$L" "$O" "$T" -scale -1 -add "$B" -scale -1 -add "$T" "$B" -multiply -add -multiply "$T" "$O" "$B" -scale -1 -add -multiply -thresh 1 inf 2 0 -add "$B" -thresh 1 inf 3 0 -add -o "$PATIENT_DIR/mask.nii.gz"; then
echo "$MEDRECNUM,$CASE_NUM" >> $CSV_FILE
mv "$OUTPUT_DIR/image.nii.gz" "$PATIENT_DIR/image_0.nii.gz"
